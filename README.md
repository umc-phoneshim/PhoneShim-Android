...
> Figma 화면 설계([디자인 링크](https://www.figma.com/design/jkpXsoAOUUuTvPZnq4QjcW))를 기준으로, 기능별 화면 수와 복잡도에 맞춰 패키지를 세분화했습니다. 특히 `setgoal`은 7단계 위저드 + 팝업 4종으로 구성되어 있어 `screen/component/viewmodel`로 분리하고, 기존에 누락되어 있던 `mypage` 패키지를 신규 반영했습니다.
...
└── ui/                            # Presentation 레이어 (Jetpack Compose)
    ...
    └── features/                  # 폰쉼 서비스의 핵심 기능별 스크린 (01~08)
        ├── auth/                  # 01~03. 스플래시 · 로그인 · 회원가입
        ├── setgoal/               # 04. 목표 설정 (7단계 위저드 + 팝업 4종)
        │   ├── screen/            #   04-1~04-7 단계별 화면
        │   ├── component/         #   접근 횟수/AI 제안 등 팝업 공용 컴포넌트
        │   └── viewmodel/         #   단계 간 공유 상태 (SetGoalViewModel)
...
Figma 프로토타입([바로가기](https://www.figma.com/design/jkpXsoAOUUuTvPZnq4QjcW))의 `01.`~`08.` 넘버링을 기준으로 정리했습니다.

| **화면 이름** | **스크린 ID** | **진입 경로** | **담당자** |
| --- | --- | --- | --- |
| **01. 스플래시 화면** | `SplashScreen` | 앱 최초 실행 시 | 이안 |
| **02. 로그인 화면** | `LoginScreen` | 스플래시 이후 자동 진입 | 이안 |
| **03. 회원가입 화면** | `SignUpScreen` | 로그인 화면 우측 하단 아이콘 선택 | 이안 |
| **04. 목표 설정 시작** | `SetGoalStartScreen` | 최초 로그인 직후 / 메인 상단 설정 아이콘 | 노뱅 |
| **04-1. 어플 선택** | `AppSelectScreen` | 목표 설정 시작 후 진입 | 노뱅 |
| **04-2. 목표 사용 시간 설정** | `UsageTimeSetScreen` | 어플 선택 완료 후 진입 | 노뱅 |
| **04-3. 접근 횟수 & 목표 설정** | `AccessGoalSetScreen` | 사용 시간 설정 후 진입 | 노뱅 |
| **04-4. 접근 횟수 설정 팝업** | `AccessCountPopup` | 04-3 화면 내 팝업 호출 | 노뱅 |
| **04-5. 목표 설정 팝업 (사용자 작성 / AI 제안)** | `GoalWritePopup` | 04-3 화면 내 팝업 호출, 탭 전환 | 노뱅 |
| **04-6. 최종 확인** | `SetGoalConfirmScreen` | 목표 작성 팝업 완료 후 진입 | 노뱅 |
| **04-7. 목표 설정 완료** | `SetGoalCompleteScreen` | 최종 확인 후 진입 | 노뱅 |
| **05. 메인 화면** | `MainScreen` | 로그인 / 목표 설정 완료 후 진입 (초기 설정 전·후 분기) | 폴 |
| **06. 리마인더 화면** | `ReminderScreen` | 하단 탭바 중앙 아이콘 선택 (초기 설정 전·후 분기) | 담담 |
| **06. 리마인더 설정 팝업** | `ReminderSetPopup` | 리마인더 화면 내 일정 추가 버튼 | 담담 |
| **07. 데일리 리포트 (타임테이블)** | `TimetableScreen` | 하단 탭바 우측 아이콘 선택 | 타로 |
| **07. 타임테이블 (사용 이유 입력)** | `UsageReasonInputScreen` | 타임테이블 항목 선택 시 | 타로 |
| **07. 타임테이블 (포인트 적립)** | `PointRewardPopup` | 사용 이유 입력 완료 후 | 타로 |
| **07. 데일리 리포트 (AI 제안)** | `ReportAiSuggestScreen` | 데일리 리포트 상단 탭 전환 | 타로 |
| **07. 데일리 리포트 (요약)** | `ReportSummaryScreen` | AI 제안 확인 후 진입 | 타로 |
| **08. 마이페이지** | `MyScreen` | 메인 화면 우측 상단 아이콘 선택 | 너울 |
| **08. 마이페이지 (사이드 슬라이드)** | `MySideMenuScreen` | 마이페이지 내 메뉴 아이콘 선택 | 너울 |
| **08. 마이페이지 (탈퇴 팝업)** | `WithdrawPopup` | 사이드 슬라이드 내 회원 탈퇴 선택 | 너울 |
...
[스플래시] ─(자동)─► [로그인] ◀──► [회원가입]
              ...
              (최초 1회) [목표 설정 시작]
                        │
                        ▼
      [어플 선택] ─► [사용 시간 설정] ─► [접근 횟수 & 목표 설정]
                                              │             │
                                     (팝업) [횟수 설정]   [목표 작성 / AI 제안]
                                              │             │
                                              ▼             ▼
                                        [최종 확인] ─► [설정 완료]
                                              │
