...
class ReportRepositoryImpl @Inject constructor(
    ...
    override suspend fun getDailyReport(date: String, isToday: Boolean): Result<DailyReport> =
        runCatchingApi {
            val usages = if (isToday) {
                ...
                reportApi.getUsageStatus().unwrap().orEmpty().map { response ->
                    ReportAppUsage(
                ...
                reportApi.getUsageLogs(date).unwrap().orEmpty().map { response ->
                    ReportAppUsage(
                        monitoredAppId = response.monitoredAppId.orEmpty(),
                        usedMinutes = response.usedMinutes ?: 0,
                        entryCount = response.entryCount ?: 0,
    ...
    override suspend fun getUsageSessions(date: String): Result<List<UsageSession>> =
        runCatchingApi {
            reportApi.getUsageSessions(date).unwrap().orEmpty().mapNotNull { response ->
    ...
    override suspend fun getAchievedDates(month: String): Result<List<String>> = runCatchingApi {
        reportApi.getUsageCalendar(month).unwrap().achievedDates.orEmpty()
    }
}
